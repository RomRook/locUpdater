...
import com.openjaw.locs.updater.algorithms.{LocationCodesInfo, LocationGeneralInfo, LocationInfo}
...
class DefaultHierarchyHelper(defaultHierarchyXML: NodeSeq) {
  ...
  val logger = Logger(LoggerFactory.getLogger("DefaultHierarchyHelper"))
  ...
  lazy val locationLookup = getLocationLookup
...
//  def extractBasicLocationInfo(locationNode: Node): BasicLocationInfo = {
//    try {
//
//      val locationId = (locationNode \ "@Id").text
//      val locationType = (locationNode \ "@LocationType").text
...
//      }
//
//      BasicLocationInfo(locationId,locationType, locationNameEN.trim, locationNameRU.trim, locationAjaxNameEN, locationAjaxNameRU, isoCode, stateInUSA.trim, locationNode)
//
//    } catch {
//      case e: Exception => throw new Exception(e.getMessage + " locationNode " + locationNode)
//    }
...
//  }
  ...
  def extractLocationCapabilities(locationNode: Node) = {
       val capabilityNodes = locationNode \\ "Capabilities" \ "Capability"
       val car = capabilityNodes.filter(n => (n \ "@Type").text == "car").headOption
       val hotel = capabilityNodes.filter(n => (n \ "@Type").text == "hotel").headOption
       val primaryGateway = capabilityNodes.filter(n => (n \ "@Type").text == "primaryGateway").headOption

      //<Capability Type="primaryGateway" Value="IAR"/>

       (car.isDefined, hotel.isDefined, primaryGateway.isDefined)
  }

//  def extractLocation(locationNode: Node): (BasicLocationInfo, LocationCodesInfo) = {
//    val basic = extractBasicLocationInfo(locationNode)
//    val codes = extractLocationCodesInfo(locationNode)
//    (basic, codes)
//  }
  ...
  def addUTSCodeSeq(originalXML: Node, utsCodeSeq: Seq[String]) = {
    val node = originalXML
    val childs: Seq[Node] = node.child
    val originalElem:Elem = Elem( node.prefix, node.label, node.attributes, node.scope, false,  childs:_*)

    val newCodes = utsCodeSeq.map(utsCode => <Code Context="UTS" Value={utsCode} ></Code>)

    originalElem match {
      case e @ Elem(_, _, _, _, childCodes @ _*) => {
        val changedNodes = childCodes.map {
          case <Codes>{ currentCodes @ _* }</Codes> => {
            val updatedCodes = (currentCodes ++ newCodes).distinct
            <Codes>{updatedCodes}</Codes>
          }
          case other => other
        }
        e.copy(child = changedNodes)
  ...
  def addPrimaryGatewayCapability(originalXML: Node) = {

    val node = originalXML
    val childs: Seq[Node] = node.child
    val originalElem:Elem = Elem( node.prefix, node.label, node.attributes, node.scope, false,  childs:_*)
    val newCapability = <Capability Type="primaryGateway" Value=""/>

    originalElem match {
      case e @ Elem(_, _, _, _, childCodes @ _*) => {
        val changedNodes = childCodes.map {
          case <Capabilities>{ currentCapabilities @ _* }</Capabilities> => {
            val pgCapability = for {
              item <- currentCapabilities
              if (item \ "@Type").text == "primaryGateway"
            } yield item
            val result = {if (pgCapability.size > 0)
              <Capabilities> {currentCapabilities}</Capabilities>
            else
              <Capabilities> {currentCapabilities ++ newCapability}</Capabilities>
            }
  ...
  def removeChildLocations(originalXML: Node, childIdSeq: List[String]) = {
    val removeIt = new RewriteRule {
      override def transform(n: Node): NodeSeq = {
        val childs: Seq[Node] = n.child
        val originalElem:Elem = Elem(n.prefix, n.label, n.attributes, n.scope, false,  childs:_*)

        originalElem match {
          case e @ Elem(_, _, _, _, childCodes @ _*) => {
                val changedNodes = childCodes.map {
                case <ChildLocations>{ currentLocations @ _* }</ChildLocations> => {
                  val filtered = currentLocations.filterNot(loc => {
                    val locationId = (loc \ "@Id").text
                    childIdSeq.contains(locationId)
                  })
                  <ChildLocations> {filtered}</ChildLocations>
                }
                case other => other
         }
         e.copy(child = changedNodes)
  ...
  val result = (new RuleTransformer(removeIt).transform(originalXML)).head
  result
 }
...
//  }
  ...
  def removePrimaryGatewayCapability(originalXML: Node) = {
    val removeIt = new RewriteRule {
      override def transform(n: Node): NodeSeq = n match {
        case e: Elem if ((e \\ "Capability" \\ "@Type").text == "primaryGateway") => {
          NodeSeq.Empty
        }
        case n => n
      }
  ...
  //todo: update only needed attr instead all
  def setLocationAsSearchable(node: Node) = {
    val locationId = (node \ "@Id").text
    val onSale = (node \ "@OnSale").text
    val tz = (node \ "@TZ").text
    val hideName   = (node \ "@HideName").text
    ...
    val searchable   = (node \ "@Searchable").text
    val indexNumber   = (node \ "@IndexNumber").text
    val ignoreDiff = (node \ "@ignoreDiff").text

    val updatedNode = node match {
      case <Location>{values @ _*}</Location> =>
        <Location Id ={locationId} OnSale={onSale} ignoreDiff="true" TZ={tz} HideName={hideName}  LocationType={locationType} Version={version} Searchable="true" IndexNumber={indexNumber} >{values}</Location>
    }
    updatedNode
  }

  //todo: update only needed attr instead all
  def setLocationActionAsDelete(node: Node) = {
    val locationId = (node \ "@Id").text
    val onSale = (node \ "@OnSale").text
    val tz = (node \ "@TZ").text
    val hideName   = (node \ "@HideName").text
    ...
    val updatedNode = node match {
      case <Location>{values @ _*}</Location> =>
        <Location Action="Delete" Id ={locationId} OnSale={onSale} ignoreDiff="true" TZ={tz} HideName={hideName}  LocationType={locationType} Version={version} Searchable={searchable} IndexNumber={indexNumber} >{values}</Location>
    }
    updatedNode
  }
