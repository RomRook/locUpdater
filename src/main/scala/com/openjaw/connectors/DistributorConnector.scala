...
import com.openjaw.locs.updater.{SenderProgress, Progress}
...
object DistributorConnector{
  ...
  class Supervisor(name: String, requestType: String, nodesToBeSent: NodeSeq, xDistEndpoint: String, xDistTimeout: Int = 180000, numberOfWorkers: Int) extends Actor {
    ...
    var totalSentCount = nodesToBeSent.size
    var totalProcessedCount = 0
    var totalSuccessCount = 0
    var totalErrorCount = 0
    logger.info(s"Number of workers (sendActor) created: $workersCount [messages count: $totalSentCount]")
    ...
    def receive = {
      ...
      case SenderProgress(processed: Int, success: Int, err: Int) => {
        ...
        //inform its parent about current progress
  ...
  def createLocationHierarchyRQ(location: NodeSeq) = {
 ...
 def createDefaultHierarchyRQ() = {
  ...
  def getDepotCodes(response: NodeSeq) = {
    ...
    val result = locationDetailNodes.map(n => {
      val depotCode = (n \ "@OJVehDepotCode").text
      // in Car Manager OJVehDepotCode is defined as 'RC-11433139'
      // supplier code is substring after sign '-'
      val rentalCarsDepotId = depotCode.substring(depotCode.indexOf("-") + 1)
      rentalCarsDepotId
    })
